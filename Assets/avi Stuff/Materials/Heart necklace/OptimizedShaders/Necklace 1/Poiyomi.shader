Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/9a5cfa97857f7aa47bbb6f6995ded428"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + (0.061 /*_AlphaMod*/));
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				#ifdef _LIGHTINGMODE_TEXTURERAMP
				poiLight.rampedLightMap = lerp(1, UNITY_SAMPLE_TEX2D(_ToonRamp, poiLight.lightMap + (-0.808 /*_ShadowOffset*/)).rgb, shadowStrength);
			...
			float3 RandomColorFromPoint(float2 rando)
			{
				...
				fixed saturation = lerp(float4(0,0.2,0,1).x, float4(0,0.2,0,1).y, rando.x);
				fixed value = lerp(float4(0.118328,0.318328,0,1).x, float4(0.118328,0.318328,0,1).y, rando.y);
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch((0.0 /*_GlitterMode*/))
				{
					...
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, (9.1 /*_GlitterAngleRange*/), (0.0 /*_GlitterSpeed*/));
					}
					else
					{
						randomRotation = randomFloat3Range(randoPoint, (9.1 /*_GlitterAngleRange*/));
					}
					float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
					finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.8 /*_GlitterBias*/)), poiCam.viewDir)), (635.0 /*_GlitterContrast*/)), 0);
					...
					finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(specAngle, (635.0 /*_GlitterContrast*/)), 0);
					glitterColor *= poiLight.directColor;
					finalGlitter *= glitterAlpha;
					break;
				}
				glitterColor *= lerp(1, poiFragData.baseColor, (0.914 /*_GlitterUseSurfaceColor*/));
				#if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				...
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * (3.3 /*_GlitterBrightness*/), finalGlitter * glitterTexture.a * glitterMask);
					poiFragData.emission += finalGlitter * glitterColor * max(0, ((3.3 /*_GlitterBrightness*/) - 1) * glitterTexture.a) * glitterMask;
				}
				else
				{
					poiFragData.emission += finalGlitter * glitterColor * (3.3 /*_GlitterBrightness*/) * glitterTexture.a * glitterMask;
				}
			}
			#endif
